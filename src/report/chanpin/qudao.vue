  ...
  <div>
    <table style="padding-left:10px;word-break: keep-all;">
      <tr>
        <td>周期:</td>
        <td>
          <selDim :itemList="comName.dateList" :slidesPerView="5"
                  @comChanged="changePageState($event,'f_dateType')"></selDim>
        </td>
      </tr>
      <tr>
        <td>渠道:</td>
        <td>
          <selDim :itemList="comName.channelList" :slidesPerView="5"
                  @comChanged="changePageState($event,'f_channel')"></selDim>
        </td>
      </tr>
      <tr>
        <td>排序:</td>
        <td>
          <selDim :itemList="comName.orderList" :slidesPerView="5"
                  @comChanged="changePageState($event,'f_order')"></selDim>
        </td>
      </tr>
      <tr>
        <td>自选:</td>
        <td>
          <selDim v-bind:itemList="comName.productList" @comChanged="changePageState($event,'f_product')"></selDim>
        </td>
      </tr>
    </table>
    <LineSplit></LineSplit>
    <table style="table-layout: fixed;width: 100%;">
      <tr id="orderG1">
        <td>
          <BtnOrder ref="order1" :item="comName.orderItem[0]"
                    @comChanged="changePageState($event,'f_orderChild')"></BtnOrder>
        </td>
        <td>
          <BtnOrder ref="order2" :item="comName.orderItem[1]"
                    @comChanged="changePageState($event,'f_orderChild')"></BtnOrder>
        </td>
      </tr>
      <tr id="orderG2" style="display: none;">
        <td>
          <BtnOrder ref="order3" :item="comName.orderItem[2]"
                    @comChanged="changePageState($event,'f_orderChild')"></BtnOrder>
        </td>
        <td>
          <BtnOrder ref="order4" :item="comName.orderItem[3]"
                    @comChanged="changePageState($event,'f_orderChild')"></BtnOrder>
        </td>
      </tr>
    </table>
